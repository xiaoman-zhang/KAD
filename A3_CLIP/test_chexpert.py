...
from dataset.test_dataset import CheXpert_Dataset_test
...
def main(args, config):
    ...
    print("Start testing")
    test(model,image_encoder, text_encoder, tokenizer, test_dataloader_chexpert,device,args,config)
...
def test(model,image_encoder, text_encoder, tokenizer, data_loader,device,args,config):
    ...
    save_result_path = os.path.join(args.output_dir,'result_chexpert_offical.csv')
    dist_csv_col =  ['metric','atelectasis', 'cardiomegaly', 'consolidation', 'edema', 'pleural effusion','mean']
    ...
    for idx in  range(1000):
        ...
        mccs_idx,threshold_idx = compute_mccs(gt_idx, pred_idx)
        F1s_idx = compute_F1s_threshold(gt_idx,pred_idx,threshold_idx)
        Accs_idx = compute_Accs_threshold(gt_idx,pred_idx,threshold_idx)
...
def get_sort_eachclass(metric_list,n_class=5):
    metric_5=[]
    metric_95=[]
    metric_mean=[]
...
def compute_AUCs(gt, pred, n_class=5):
    metrics = {}
    AUROCs = []
    AUROCs.append('AUC')
    gt_np = gt 
...
def compute_F1s_threshold(gt, pred,threshold,n_class=5):
    bert_f1 = 0.0
    gt_np = gt 
    pred_np = pred 
...
def compute_Accs_threshold(gt, pred,threshold,n_class=5):
    gt_np = gt 
    pred_np = pred 
...
def compute_mccs(gt, pred, n_class=5):
